...
# # Speed is locked between 0 and 300 pixels/s -> 6pixels/sim
# # we can at maximum accelerate decelerate at +- 50s^2 -> we can add/subtract at maxomum .02 pixels from our velocity in a simulation loop
# # 5 pixels = 1m
...
if 1:
  image = img.imread('car racing project/racetrack2/racetrack.png')
  raceTrack = np.matrix([[0 if q[0] <=.2 else 1 for q in c] for c in image])
else:
  image = img.imread('car racing project/racetrack1/racetrack.png')
...
class RaceTrack(Car):
  racetrack = raceTrack
  carrender = np.zeros(np.shape(raceTrack))
  num_cars = 1
  cars = [Car() for q in range(len(num_cars))]
  
  def draw_cars():
    carrender = raceTrack.copy()
